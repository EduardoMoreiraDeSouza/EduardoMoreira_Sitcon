...
class AlterarSolicitacoes extends ExecutarQueryMysql
{
	public function __construct()
	{
		...
		$execucaoPacientes = $this -> ExecutarQueryMysql("SELECT * FROM teste_sitcon.pacientes WHERE nome LIKE '$nomePaciente' AND dataNasc LIKE '$dataNasc' AND cpf LIKE '$cpf' AND status LIKE 'ativo';");
		...
		$execucaoProfissioanl = $this -> ExecutarQueryMysql("SELECT * FROM teste_sitcon.profissional WHERE id LIKE '$profissional' AND status LIKE 'ativo';");
		...
		$execucaoProcedimentos = $this -> ExecutarQueryMysql("SELECT * FROM teste_sitcon.procedimentos WHERE id LIKE '$procedimentos' AND status LIKE 'ativo';");
		...
		if ($dadosProcedimentos['tipoSolicitacao_id'] == $dadosTipoSolicitacao['id']) {
			...
			$selecionarSolicitacaoPaciente = $this -> ExecutarQueryMysql("SELECT * FROM teste_sitcon.solicitacoes WHERE id_paciente LIKE '" . $dadosPaciente['id'] . "';");
